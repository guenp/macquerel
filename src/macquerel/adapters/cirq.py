...
def from_cirq(cirq_circuit) -> Circuit:
    ...
    """
    try:
        import cirq  # ty: ignore[unresolved-import]
    ...
    for moment in cirq_circuit:
        for op in moment.operations:
            ...
            # cirq.rx/ry/rz(rads) return X/Y/ZPowGate instances with a -0.5 global
            # shift; the rotation angle is exponent * pi. They are not types, so
            # they must be matched on global_shift, before the plain-gate checks.
            if isinstance(gate, cirq.MeasurementGate):
                qc.measure(qubits)
            elif isinstance(gate, cirq.HPowGate) and gate.exponent == 1:
                qc.h(qubits[0])
            elif isinstance(gate, cirq.XPowGate) and gate.global_shift == -0.5:
                qc.rx(qubits[0], gate.exponent * math.pi)
            elif isinstance(gate, cirq.YPowGate) and gate.global_shift == -0.5:
                qc.ry(qubits[0], gate.exponent * math.pi)
            elif isinstance(gate, cirq.ZPowGate) and gate.global_shift == -0.5:
                qc.rz(qubits[0], gate.exponent * math.pi)
